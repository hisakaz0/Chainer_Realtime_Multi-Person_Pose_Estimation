...
def get_bbox_img(img, bbox, label, score, label_names=voc_bbox_label_names):
    ...
    font_size = 14 * font_scale

    h, w, _ = img.shape

    for bb, lb, sc in zip(bbox, label, score):
        x_s, y_s = bb[1], bb[0]
        x_e, y_e = bb[3], bb[2]
        x_s = 0 if x_s < 0 else x_s
        y_s = 0 if y_s < 0 else y_s
        x_e = w if x_e > w else x_e
        y_e = h if y_e > h else y_e
        cv2.rectangle(img, (x_s,y_s), (x_e,y_e), color, line_width)
        ...
        text = "{}({}%)". format(label_names[lb], int(sc*100))
        cv2.putText(img, text, (int(x_s), int(y_s)),
                font, font_scale, (255, 255, 255), font_weight)
...
if __name__ == '__main__':
    ...
    ssd_model = SSD300(n_fg_class=len(voc_bbox_label_names), pretrained_model='voc0712')
    ...
    base = [l for l in open(args.image_list).read().strip().split('\n')]
    for i, line in enumerate(base):
        image_path = os.path.join(args.image_dir, line)
        image = utils.read_image(image_path, color=True)
        cv_image = cv2.imread(image_path)
        bboxes, labels, scores = ssd_model.predict([image])
        bbox, label, score = bboxes[0], labels[0], scores[0]
        drawed_image = get_bbox_img(cv_image, bbox, label, score,
                label_names=voc_bbox_label_names)

        poses, _ = pose_detector(cv_image)
        drawed_image = draw_person_pose(drawed_image, poses)

        res_image_path = os.path.join(args.out, line)
        cv2.imwrite(res_image_path, drawed_image)
        sys.stdout.write("\r{}/{}". format(i+1, len(base)))
